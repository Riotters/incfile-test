...
class RegistrationPage extends React.Component {
  ...
  render() {
    const priceVariants =
      variants.hasOwnProperty(this.state.state) &&
      variants[this.state.state].hasOwnProperty(this.state.entity)
        ? variants[this.state.state][this.state.entity]
        : null;

    const fillingRequirements =
      priceVariants != null
        ? variants[this.state.state][this.state.entity]["requirements"]
        : null;
    const services =
      priceVariants != null
        ? variants[this.state.state][this.state.entity]["services"]
        : null;
    ...
    return (
      <Layout>
        <SEO
          title="Register Your LLC, S Corp, C Corp, or Nonprofit | Incfile"
          description="Incfile's business formation services are easy and affordable. Start your business today for as little as $0 + state fees. Learn more. "
        />
        <HeaderSection
          content={top}
        ...
        {services != null && <TrustedSection content={trusted} />}
      </Layout>
    );
  }
}
