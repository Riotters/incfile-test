	...
	useEffect(() => {
		if (checked === id) {
			const targetElement = document.getElementById(
				"descriptionInsideRadio-" + id
			);
			const closestWrapper = targetElement.closest(
				"." + Wrapper.styledComponentId
			);
			const parentElement = targetElement.closest(
				`div[class^="form-content__Wrapper-"]`
			);
			const parentStyle = getComputedStyle(parentElement);
			const currentArrow = targetElement.querySelector(
				`[class="arrowInsideRadioButton"]`
			);
			const allRadioWrapper = parentElement.querySelectorAll(
				`[class^="radio__Wrapper-"]`
			);
			const allRadioDescriptionWrapper = parentElement.querySelectorAll(
				`[id^="descriptionInsideRadio-"]`
			);
			...
			function handleTabs() {
				...
				if (description) {
					currentArrow.style.left =
						closestWrapper.offsetLeft + closestWrapper.offsetWidth / 3 + "px";
					targetElement.style.top =
						closestWrapper.offsetTop + closestWrapper.offsetHeight + 20 + "px";
					targetElement.style.left = parseInt(parentStyle.paddingLeft) + "px";
					targetElement.style.visibility = "visible";
					targetElement.style.width =
						parentElement.offsetWidth -
						parseInt(parentStyle.paddingLeft) -
						parseInt(parentStyle.paddingRight) +
						"px";
					closestWrapper.style.marginBottom =
						targetElement.offsetHeight + 20 + "px";
				}
			}
	...
	return (
		<Wrapper className={className}>
			<Input
				...
				type="radio"
				name={name}
				checked={onClick ? checked === id : checked}
				value={value}
				onClick={onClick}
				ref={radioRef}
			/>
