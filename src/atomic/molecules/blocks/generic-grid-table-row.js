...
const GenericGridTableRow = ({ className, content, settings, columns }) => (
	<Wrapper className={className} columns={columns}>
		{content.map((item, i) => (
			<Cell
				textCenter={item.textCenter ?? false}
				style={item.style ?? {}}
			...
			>
				...
				{!item.header && item.type === "list" && (
					<ListItem bottomMargin="0">{item.label}</ListItem>
				)}
				{item.subcontent && (
					<Tab>
						...
						{item.variant && (
							<VariantWrapper>
								{item.variant.icon && (
									<ImageWrapper>
										<Image
											filename={item.variant.icon}
											alt={item.variant.label}
										/>
									</ImageWrapper>
								)}
								<Paragraph bottomMargin={0} style={item.variant.style}>
				...
				{!item.header && item.type === "icon" && (
					<div className="flex-center">
						{item.icon && <Image filename={item.icon} alt={item.label} />}
					</div>
				)}
