...
const FeesTable = () => {
	...
	const fetchData = async () => {
		const response = await fetch(
			`${process.env.INCFILE_API_URL}/getStatePrices`
	...
	const handleSortPrice = (entity) => {
		let reSort = [...prices].sort((a, b) => {
			...
			if (entity === "CCorp") {
				return ccorpSort
					? a.CCorporation - b.CCorporation
					: b.CCorporation - a.CCorporation;
			}

			if (entity === "SCorp") {
				return scorpSort
					? a.SCorporation - b.SCorporation
					: b.SCorporation - a.SCorporation;
			}
				...
				<FeesTableWrapper>
					<TableRow>
						...
						<TableCell
							style={{ cursor: `pointer` }}
							onClick={() => {
								setCCorpSort(!ccorpSort);
								handleSortPrice(`CCorp`);
							}}
						>
							C-Corp
						</TableCell>
						<TableCell
							style={{ cursor: `pointer` }}
							onClick={() => {
								setSCorpSort(!scorpSort);
								handleSortPrice(`SCorp`);
							}}
						>
							S-Corp
						</TableCell>
					...
					{prices.length &&
						prices.map((price, i) => {
							return <FeesTableRow price={price} i={i} key={i} />;
						})}
				</FeesTableWrapper>

				<Paragraph mixed big>
					Compare state filing times side by side using our{" "}
					<Link to={`/compare-state-filing-times/`}>comparison tool</Link>.
				</Paragraph>
			</Container>
		</Wrapper>
	);
