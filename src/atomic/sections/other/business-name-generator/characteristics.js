...
const Characteristics = ({ className, content }) => (
    <Wrapper>
        ...
        <LeftImageRightContent image={content.image}>
            ...
            <Paragraph big mixed>{parse(content.text3)}</Paragraph>

            <ListContainer>
                {content.list.map(item => (
                    <Whitebox>
                        <CheckSVG/>
                        <Paragraph mixed bottomMargin={0}>{parse(item)}</Paragraph>
                    </Whitebox>
                ))}
            </ListContainer>

            <Heading size={3}>{content.header3}</Heading>
            <Paragraph big mixed>{parse(content.text4)}</Paragraph>
            ...
            <Heading size={2} template={3} left bottomMargin={48}>{content.header4}</Heading>
