...
const BusinessNameSearchForm = ({
	...
	const [businessNameInState, setBusinessNameState] = React.useState(
		propState || ""
	);
				...
				<Wrapper className={className} noMaxWidth={noMaxWidth}>
					{curvePosition === "bottomRight" && (
						<Curve
							bottom="-25"
							right="-29"
							color={curveColor ?? color.orange1}
							rotate={curveRotate ?? 0}
						>
							<CurveSVG />
						</Curve>
					)}
					...
					<Heading size={3} template={5} bottomMargin="16">
						{" "}
						Please enter the business name information
					</Heading>
					<Label
						htmlFor="entity_name"
						content={{ label: `Entity Name` }}
						bottomMargin="24"
					>
						<Input
					...
					<Flex>
						<Label
							htmlFor="type"
							content={{ label: `Entity Type` }}
							bottomMargin="16"
							bottomDirection="bottom row"
						>
							<Radio
								content="LLC"
								checked
								...
								id="bns-llc"
								value="LLC"
								inputRef={register({ required: `Please select Entity Type` })}
							/>
							<Radio
								content="Corporation"
								name="entity_type"
					...
					<Label
						className={propState ? "hide" : ""}
						htmlFor="business_name_search_state"
						content={{ label: `Entity State` }}
						bottomMargin="32"
					>
						<Controller
							control={control}
							...
							render={() => (
								<Dropdown
									...
									onChange={(option) => {
										setValue("business_name_search_state", option.value, {
											shouldValidate: true,
										});
									}}
									placeholder="Select"
						...
						{errors.business_name_search_state && (
							<span className="error__info">
								{errors.business_name_search_state.message}
							</span>
						)}
					</Label>
					...
					<Heading size={3} template={5} bottomMargin="16">
						{" "}
						Let us know where we can let you know if your business name is
						available
					</Heading>
					<Grid>
						<Label
							htmlFor="first-name"
							content={{ label: `First Name` }}
							bottomMargin="32"
						>
							<Input
					...
					<Button
						type="submit"
						content={{
							text: isSubmitting ? `Processing...` : `Check Name Avaliability`,
						}}
						theme="primary56"
						marginSM="48px auto 0"
					/>
				</Wrapper>
			</form>
