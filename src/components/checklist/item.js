...
class Item extends React.Component {
	...
	handleCollapse = (event) => {
		...
		const height =
			...
			headerMarginBottom +
			descritionMarginBottom;
		this.setState(
			(prevState) => ({ expanded: !prevState.expanded }),
			() => {
				this.props.onChangeHeight(
					this.props.id,
					this.state.expanded ? height : this.props.initHeight
				);
			}
		);
	};
	...
	shouldBeExpanded = () => {
		return (
			this.state.expanded && !(this.props.isStack && this.props.isCompleted)
		);
	};
	...
	render() {
		...
		return (
			<Wrapper
				className={isStack && isCompleted ? className + " stacked" : className}
				ref={this.itemRef}
				index={index}
				total={meta.completed.length}
			>
				<ItemHead
					className={this.shouldBeExpanded() ? "expanded" : ""}
