...
class Checklist extends React.Component {
	...
	getMeta() {
		...
		return {
			completed: {
				items: completed,
				height: this.state.stack
					? this.baseHeight + 20
					: completed.length > 0
					? _.sumBy(completed, "height")
					: 0,
	...
	toggleClass(id) {
		...
		this.setState({
			items: updatedItems,
			shadowCards: this.calcShadowCardsCount(
				this.state.stack,
				completed.length
			),
		});
	...
	handleChangeItemHeight = (id, height) => {
		...
		const updatedItems = items.map((item) => ({
			...item,
			height: item.id === id ? height : item.height,
	...
	render() {
		...
		const doneItems = this.state.items
			.filter((item) => item.isCompleted)
			.map((item) => {
				let isLastCompleted =
					completed.length > 0 ? _.last(completed).id === item.id : false;
				return (
					<Item
						id={item.id}
		...
		const undoneItems = this.state.items
			.filter((item) => !item.isCompleted)
			.map((item) => {
				let isLastCompleted =
					completed.length > 0 ? _.last(completed).id === item.id : false;
			...
			<Wrapper>
				<Header>
					<HeaderIcon
						...
						shadowColor={this.props.iconShadowColor}
					>
						<this.props.icon />
					</HeaderIcon>
					<HeaderContent>
				...
				<List>
					...
					<Separator>
						...
						<ShowUncompleted
							show={completed.length > 0}
							onStackItems={this.stackItems}
						>
							show Less
						</ShowUncompleted>
					</Separator>
					<Done
						className={this.state.stack ? "stacked" : ""}
						style={
							this.state.stack
								? { height: itemHeight + 20 + doneItems.length * 10 + "px" }
								: {}
						}
					>
						{doneItems}
