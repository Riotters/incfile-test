...
const CertificateCard = ({
	...
	const [stateFormationOptions, setStateFormationOptions] = useState(
		dropdownTwoOptions
	);
	...
	const checkForeignQualificateState = (option) => {
		...
		if (stateOfFQ === entityState) {
			alert(
				`The State of Incorporation and State of Foreign Qualification should not same`
			);
			setExtraFee("Plus State Filing Fees");
			setCompState("");
		} else {
			...
			fetchData(endpoint).then((data) => {
				setExtraFee(
					`+$${data[priceColumnField]} ${shortState(stateOfFQ)} State Fee`
				);
			});
		}
	};
	...
	const handleOrderNow = () => {
		if (typeof window !== "undefined") {
			let entityTypeItem = entityOptions.filter(
				(item) => item.value === entityTypeSelected
			);
		...
		<Wrapper className={className}>
			...
			{dropdownOnePlaceholder && (
				<Label className="label">
					...
					<Dropdown
						className="dropdown"
						placeholder={dropdownOnePlaceholder}
						options={entityOptions}
						onChange={_onChangeEntityType}
					/>
				</Label>
			)}
			{dropdownTwoPlaceholder && (
